...
def Controle(posX, posY, tamX, tamY):
    ...
    print(str(time.time() - tempo_cap) + ":" + str(erro_X) + ":" + str(erro_Y) + ":" + str(controle_X) + ":" + str(controle_Y) + ":" + str(angulo_X) + ":" + str(angulo_Y))
...
while True:
	...
	if len(contornos) > 0:
		...
		#Testa o raio do circulo para evitar ruídos
		if raio > 10:
			#Começa o controle e captura de dados
			if captura == 1:
	...
	cv.putText(frame, "FPS: " + fps, (0, altura-10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255))
	...
	if tecla == 103:
		captura = 1
		print("Tempo:Erro X:Erro Y:Controle X:Controle Y:Angulo X:Angulo Y")
		tempo_cap = time.time()	
	if tecla == 115:
		captura = 0
